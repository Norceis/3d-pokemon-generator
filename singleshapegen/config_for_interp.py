...
class Config(object):
    ...
    def _add_network_config(self, parser):
        ...
        group.add_argument('--G_struct', type=str, default="triplane", choices=["triplane", "conv3d", 'triplane_2_heads'], help='generator structure')
    ...
    def _add_testing_config(self, parser):
        ...
        group.add_argument('--mode', type=str, default='rand', choices=['rand', 'rec', 'interp', 'interp_pokemon'], help="inference mode")
    ...
    def _add_second_network(self, parser):
        """general arguments"""
        group = parser.add_argument_group('second_network')
        group.add_argument('--proj_dir_second_network', type=str, default="checkpoints", help="a folder where experiment logs will be saved")
        group.add_argument('--tag_second_network', type=str, required=True, help="tag for this experiment run")
        # group.add_argument('-g', '--gpu_ids', type=int, default=0, help="which gpu to use. -1 for CPU.")

        group_1 = parser.add_argument_group('testing_second_network')
        group_1.add_argument('--ckpt_second_network', type=int, default=None, help="checkpoint at scale x. By default, use the highest scale.")
        group_1.add_argument('--mode_second_network', type=str, default='rand', choices=['rand', 'rec', 'interp'], help="inference mode")
        group_1.add_argument("--resize_second_network", nargs="*", type=float, default=[1, 1, 1], help="resize factor along each axis")
        group_1.add_argument('--upsample_second_network', type=int, default=1, help="upsample scale factor. >1 for higher resolution output")
        group_1.add_argument('--n_samples_second_network', type=int, default=1, help="number of samples to generate")
        group_1.add_argument('--no_binsecond_network', dest='bin', action='store_false', help='do not binarize output')
        group_1.set_defaults(bin=True)

...
from .model_utils import calc_gradient_penalty, set_require_grads, slice_volume_along_xyz, TrainClock, calc_gradient_penalty_2_heads


def _check_bounds(coordinates, lower_limit, upper_limit):
    new_coordinates = []
    for crd in coordinates:
        if crd < lower_limit:
            crd = lower_limit
        elif crd > upper_limit:
            crd = upper_limit

        new_coordinates.append(crd)
    return new_coordinates


def _step_cut_point(cut_point, dimension_to_scale, half_window_size):
    return np.random.randint(int(cut_point - np.ceil(dimension_to_scale * half_window_size)),
                             int(cut_point + np.ceil(dimension_to_scale * half_window_size)) + 1)
...
class SSGmodelBase(ABC):
    ...
    def load_ckpt_second(self, n_scale: int, model_dir, name="latest", ):
        """load saved checkpoint"""
        load_path = os.path.join(model_dir, f"scale{n_scale}_{name}.pth")
        if not os.path.exists(load_path):
            raise ValueError(f"Checkpoint {load_path} not exists.")
        print(f"Load checkpoint from {load_path}.")
        checkpoint = torch.load(load_path)
    ...
    def _critic_wgan_iteration_2_heads(self, real_data: torch.Tensor, second_data: torch.Tensor):
        """critic (discriminator) training iterations
        ...
        # input real and generated data to netD
        summed_data = (real_data, second_data)
        d_real = self.netD(summed_data)
        d_generated = self.netD((generated_data.detach(), generated_data.detach()))
        ...
        # get gradient penalty
        if self.config.lambda_grad:
            gradient_penalty = calc_gradient_penalty_2_heads(self.netD,
                                                     real_data, second_data, generated_data, self.device) * self.config.lambda_grad
            loss += gradient_penalty
    ...
    def _generator_iteration_2_heads(self, real_data: torch.Tensor, second_data: torch.Tensor):
        """generator training iterations
        ...
        if self.config.alpha:
            generated_data_rec = self._draw_fake_in_training('rec')
            loss_recon = (F.mse_loss(generated_data_rec, real_data) + F.mse_loss(generated_data_rec,
                                                                                 second_data)) / 2 * self.config.alpha
            loss += loss_recon
    ...
    def _updateStep_2_heads(self, real_data: torch.Tensor, second_data: torch.Tensor):
        """a training iteration (step), including both G and D updates"""
        self._update_loss_dict(None)
        self.netD.train()
        self.netG.train()
        ...
        # (1) Update D network: maximize D(x) + D(G(z))
        for j in range(self.config.Dsteps):
            self._critic_wgan_iteration_2_heads(real_data, second_data)

        # (2) Update G network: maximize D(G(z)) + rec(G(noise_opt), real_data)
        for j in range(self.config.Gsteps):
            self._generator_iteration_2_heads(real_data, second_data)
    ...
    def _train_single_scale_2_heads(self, real_data: torch.Tensor, second_data: torch.Tensor):
        """train current scale for n iterations"""
        print(f"scale: {self.scale}, real shape dimensions: {real_data.shape}, noise amp: {self.noiseAmp_list[-1]}")
        pbar = tqdm(range(self.config.n_iters), desc=f"Train scale {self.scale}")
        self.prev_opt_feats = None  # buffer of prev scale features for reconstruction
        for i in pbar:
            losses = self._updateStep_2_heads(real_data, second_data)
            pbar.set_postfix(OrderedDict({k: v.item() for k, v in losses.items()}))
    ...
    def _train_single_scale_interpolated_crossed_samples(self, real_data: torch.Tensor, second_data: torch.Tensor):
        """train current scale for n iterations"""
        print(f"scale: {self.scale}, real shape dimensions: {real_data.shape}, noise amp: {self.noiseAmp_list[-1]}")
        pbar = tqdm(range(self.config.n_iters), desc=f"Train scale {self.scale}")
        self.prev_opt_feats = None  # buffer of prev scale features for reconstruction
    ...
    def train_2_heads(self, real_data_list: list, second_data_list: list):
        """train on a list of multi-scale 3D shapes (coarse-to-fine).
        ...
        for s in range(self.scale, self.n_scales):
            ...
            # train for current scale
            self._train_single_scale_2_heads(self.real_list[s], self.second_list[s])
            self.scale += 1
    ...
    def _set_second_data(self, second_data_list: list):
        ...
        self.second_list = [torch.tensor(x, dtype=torch.float32, device=self.device).unsqueeze(0).unsqueeze(0) for x in
                            second_data_list]
...
class SSGmodelBase_2_heads(ABC):
    """Base class for single shape generative model"""
    ...
    def __init__(self, config):
        ...
        self.scale = 0
        self.netD = get_network(config, 'D2').to(self.device)
        self.netG = get_network(config, 'G').to(self.device)
        self.noiseOpt_init = None  # fixed noise at first scale
        self.noiseAmp_list = []  # gaussian noise std for each scale
        self.real_sizes = []  # real data spatial dimensions
    ...
    def _critic_wgan_iteration_2_heads(self, real_data: torch.Tensor, second_data: torch.Tensor):
        ...
        # input real and generated data to netD
        d_real = self.netD(real_data)
        d_second_real = self.netD(second_data)
        d_generated = self.netD(generated_data.detach())

        # adversarial loss
        loss_r = -d_real.mean()
        loss_sr = -d_second_real.mean()
        loss_f = d_generated.mean()
        loss = loss_f + loss_r + loss_sr
        ...
        # record loss value
        loss_values = {'D': loss.data.item(), 'D_r': loss_r.data.item(), 'D_f': loss_f.data.item(), 'D_sr': loss_sr.data.item()}
        if self.config.lambda_grad:
            loss_values.update({'D_gp': gradient_penalty.data.item()})
        self._update_loss_dict(loss_values)
